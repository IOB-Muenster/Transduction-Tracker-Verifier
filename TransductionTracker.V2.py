...
from modules.core_functions_TransductionTracker_V2 import *
...
def main():
    ...
    my_parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent("""\
                                    ...
                                    """),
        usage=textwrap.dedent("""python TransductionTracker.V2.py -r <repeat_annotation_path> -g <reference_genome> -s <segmental_duplications (optional)> -o <output_directory> -u <upstream_flank> -d <downstream_flank> -a <Alu_type> -c <cpu_number>
                              """),
        epilog=textwrap.dedent("""
                               Example: python TransductionTracker.V2.py -r repeats.bed -g genome.fasta -s seg_dup.bed -o output_directory -u 100 -d 4500 -a AluY -c 4
                               """
        )
        )
    ...
    print(f"{Style.BRIGHT}{Fore.YELLOW}Initializing TransductionTracker...")
    time.sleep(3)
    ...
    output_path_dir = Path(args.output_directory).resolve() #"resolve" converts the provided dir to an absolute path
    ...
    #~~~ Notifying the user of the analysis start
    ...
    print(f"{Style.BRIGHT}{Fore.GREEN}Identified {len(te_list)} full-length {TE_type}s for analysis (note: full-length elements overlapping with segmental duplications are excluded if the SegDup annotation is provided.")
    time.sleep(1)
    ...
    #~~~  A variable keeping the column names for the output     
    ...
    # a list that keeps track of those TEs whose flanking DNA is unknown (in case the ref genome is not T2T)
    up_downstream_gap = [] #contains TEs whose up/downstream sequences contain  "N"
    
    #~~~ Preparing the delayed funcs for parallelization
    #~~~ Extracting the flanking DNA
    delayed_fucns = []
    for locus in te_list:
        ...
        if strand == "+":
            ...
            if ("N" in f_upstream_seq) or ("N" in f_downsream_seq):
                ...
                try:
                    delayed_fucns.append(delayed(find_putative_TSD_f_V5)(chrom, start, end, strand, te_type, f_upstream_seq, f_downsream_seq, up_flank, down_flank))
                except IndexError:
                    pass
        else:  #analyzing the TEs on the reverse strand
            ...
            if ("N" in r_upstream_seq) or ("N" in r_downsream_seq):
                ...
                try:
                    delayed_fucns.append(delayed(find_putative_TSD_r_V5)(chrom, start, end, strand, te_type, r_upstream_seq, r_downsream_seq, up_flank, down_flank))
                except IndexError:
                    pass
        stack.pop()
    ...
    print(f"{Style.BRIGHT}{Fore.GREEN}The TSD detection step is completed. Generating output files...")
    time.sleep(10)
    
    #~~~ Writing those TEs not analyzed due to gap
    ...
    #~~~ Writing the final output (detected TSDs)
    ...
    print(f'{Style.BRIGHT}{Fore.GREEN}Initialzing transdcution detection step...') 
    time.sleep(10)
    ...
    print(f'{Style.BRIGHT}{Fore.GREEN}Transduction detection completed.') 
    time.sleep(10)
